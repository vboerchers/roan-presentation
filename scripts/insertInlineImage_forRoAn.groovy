...
class MyImage {
	static String invalidImageFile=Message.textUtils.getText(
		"addons.inlineImage.invalidImageFile",
		"invalid image file :"
	)
	static String cancel=Message.textUtils.getText(
		"addons.inlineImage.cancel",
		"Cancel"
	)
	...
	def imgFromUrl(theUrl) {
		...
		try {
			...
			logger.warn(invalidImageFile + theUrl, e)
		}
	}
...
def btCancel = builder.button(MyImage.cancel)

/*********************************************************************************************************
* New type of dialog for the preview pane
*/
class PPane extends JDialog {
	static Boolean roan_default=Preferences.getBooleanProp(
		"addons.inlineImage.defaultDetails", //inits with details in stead of node core
		false
		)
	
	static String mustBeSaved=Message.textUtils.getText(
		"addons.inlineImage.mustBeSaved",
		"The map must be saved if you want to use relative paths"
	)
	static String saveClipboardImageAsTitle=Message.textUtils.getText(
		"addons.inlineImage.saveclipboardImageAsTitle",
		"Save clipboard image as"
	)
	static String imageFiles=Message.textUtils.getText(
		"addons.inlineImage.imageFiles",
		"Image files"
	)
	static String insertModifyImage=Message.textUtils.getText(
		"addons.inlineImage.insertModifyImage",
		"Insert/Modify Image"
	)
	static String pasteImageFromClipboardTitle=Message.textUtils.getText(
		"addons.inlineImage.pasteImageFromClipboardTitle",
		"Paste image from Clipboard"
	)
	static String la_width=Message.textUtils.getText(
		"addons.inlineImage.la_width",
		"Width:"
	)
	static String la_height=Message.textUtils.getText(
		"addons.inlineImage.la_height",
		"Height:"
	)
	static String imagePath=Message.textUtils.getText(
		"addons.inlineImage.imagePath",
		"image path"
	)
	static String yourLink=Message.textUtils.getText(
		"addons.inlineImage.yourLink",
		"your link"
	)
	static String asLegend=Message.textUtils.getText(
		"addons.inlineImage.asLegend",
		"as legend"
	)
	static String noLink=Message.textUtils.getText(
		"addons.inlineImage.noLink",
		"no link"
	)
	static String proportionalImageResizing=Message.textUtils.getText(
		"addons.inlineImage.proportionalImageResizing",
		"Proportional image resizing:"
	)
	static String rb_core=Message.textUtils.getText(
		"addons.inlineImage.rb_core",
		"Core"
	)
	static String rb_details=Message.textUtils.getText(
		"addons.inlineImage.rb_details",
		"Details"
	)
	static String rb_note=Message.textUtils.getText(
		"addons.inlineImage.rb_note",
		"Note"
	)
	static String rb_extension=Message.textUtils.getText(
		"addons.inlineImage.rb_extension",
		"Extension"
	)
	static String la_target=Message.textUtils.getText(
		"addons.inlineImage.la_target",
		"Target:"
	)
	static String linkTargetLabel=Message.textUtils.getText(
		"addons.inlineImage.linkTargetLabel",
		"Link target:"
	)
...
* Main Dialog box
*/
def dialogue = builder.dialog(title:PPane.insertModifyImage, id:'insertImage', modal:true,
	...
	panel() {
		...
		panel(alignmentX:0f) { 
			...
			*/
			button(actionPerformed:{ 
				...
				FileNameExtensionFilter imagesFilter = new FileNameExtensionFilter(PPane.imageFiles, "gif", "jpg", "jpeg", "png", "svg");
		...
		*/
		panel(alignmentX:0f) { 
			BufferedImage image;
			flowLayout(alignment:FL.LEFT)
			button(text: PPane.pasteImageFromClipboardTitle, 
			actionPerformed:{
				...
				try {
					if (clipboard.isDataFlavorAvailable(DataFlavor.imageFlavor)) { //contains an image
						...
						save.setDialogTitle(PPane.saveClipboardImageAsTitle)
		...
		*/
		panel(alignmentX:0f) {
			flowLayout(alignment:FL.LEFT)
			label(PPane.linkTargetLabel)
			buttonGroup().with { urlGroup ->
				noLink    = radioButton(id:'noLinkRB', text:PPane.noLink, buttonGroup:urlGroup, selected:PPane.roan_default)
				imagePath = radioButton(id:'urlRB', text:PPane.imagePath, buttonGroup:urlGroup, selected:!PPane.roan_default)
				customUrl = radioButton(id:'customUrlRB', text:PPane.yourLink, buttonGroup:urlGroup)
			}
			legend = checkBox(id:'legendCB', label:PPane.asLegend, selected:false)
		}
		...
		*/
		panel(alignmentX:0f) { 
			flowLayout(alignment:FL.LEFT)
			label(PPane.proportionalImageResizing)
		}
		...
		*/
		panel(alignmentX:0f) {
			flowLayout(alignment:FL.LEFT)
			label(PPane.la_target)
			buttonGroup().with { group ->
				radioButton(id:'core', text:PPane.rb_core, selected:!PPane.roan_default,buttonGroup:group)
				radioButton(id:'details', text:PPane.rb_details, selected:PPane.roan_default, buttonGroup:group)
				radioButton(id:'note', text:PPane.rb_note, buttonGroup:group)
				radioButton(id:'extObj', text:PPane.rb_extension, buttonGroup:group)
			}
		}
...
String findRelativePath(File baseFile, String path) {
	if (baseFile == null) {
		ui.errorMessage(PPane.mustBeSaved)
		return path
	}
